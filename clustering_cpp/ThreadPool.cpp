...
ThreadPool::ThreadPool(CGConfig *cg_config, BBConfig *bb_config, SharedData *shared_data, CGData *input_data, int n_thread) {

    // This returns the number of threads supported by the system.
    // auto numberOfThreads = std::thread::hardware_concurrency();
    int numberOfThreads = n_thread;

    this->cg_config = cg_config;
    this->bb_config = bb_config;
...
void ThreadPool::doWork(int id) {
    ...
    while (true) {
        ...
        std::vector<JobData *> jobs = solve_child_problem(job, input_data, shared_data, cg_config, bb_config);
